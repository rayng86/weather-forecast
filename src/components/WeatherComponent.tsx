...
const CityInputField = ({
  ...
  >
    ...
    />
    <button title="Set" onClick={updateCityForecast}>
      Set
    </button>
    <button
      title="Switch to Fahrenheit"
      onClick={() => setMeasurementType('F')}
    >
      F
    </button>
    <button title="Switch to Celsius" onClick={() => setMeasurementType('C')}>
      C
    </button>
  </div>
);
...
const CurrentLocationComponent = ({
  ...
  <>
    <div style={{ display: 'flex', alignItems: 'center' }}>
      ...
      >
        <button className="text-button" onClick={useGeolocation}>
...
const DisplayWeatherWrapper = () => {
  ...
  const [measurementType, setMeasurementType] = useState<MeasurementTypes>(
    DEFAULT_CONFIG.measurementType
  );
    ...
    if ('geolocation' in navigator) {
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        (err) => {
          ...
          setGeoErrorMsg(
            `Error Reason: ${err.message}. Please enable browser location access and try again.`
          );
          setGeoLoadingSpinnerStatus(false);
        }
      );
